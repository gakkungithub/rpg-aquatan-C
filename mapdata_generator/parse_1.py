...
class ASTtoFlowChart:
    ...
    # 変数の型を取得
    def get_var_type(self, type: ci.Type, type_dict):
        # ポインタ型なら中身も
        if type.kind == ci.TypeKind.POINTER:
            pointee = type.get_pointee()
            children_type_dict = {}
            self.get_var_type(pointee, children_type_dict)
            type_dict['*'] = {"type": type.spelling, "children": children_type_dict}

        # 配列型なら要素型も
        elif type.kind == ci.TypeKind.CONSTANTARRAY:
            elem = type.get_array_element_type()
            children_type_dict = {}
            self.get_var_type(elem, children_type_dict)
            type_dict['[]'] = {"type": type.spelling, "children": children_type_dict}
        ...
        # 構造体の場合はフィールド列挙
            ...
            for field in type.get_fields():
                children_type_dict = {}
                self.get_var_type(field.type, children_type_dict)
                type_dict[field.spelling] = {"type": type.spelling, "children": children_type_dict}
    ...
    def parse_var_decl(self, cursor: ci.Cursor, nodeID, edgeName=""):
        ...
        type_dict = {}
        self.get_var_type(cursor.type, type_dict)
        print(f"{cursor.spelling}: ", type_dict)
