...
class ASTtoFlowChart:
                        ...
                        for i, member in enumerate(members):
                            if i < memberNum:
                                ...
                                if len(self.expNode_info[f'"{memberNodeID}"'][2]) != 0:
                                    if self.expNode_info[f'"{memberNodeID}"'][0] in ["malloc", "realloc"]:
                                        self.expNode_info[f'"{memberNodeID}"'][2][-1]["vartype"] = member[1]
                                        self.expNode_info[f'"{memberNodeID}"'][2][-1]["varname"] = ".".join([cursor.spelling, member[0]])
    ...
    def parse_do_stmt(self, cursor, nodeID, edgeName=""):
        ...
        self.condition_move[f'"{falseNodeID}"'] = ('doWhileFalse', [cr.location.line, *self.expNode_info[f'"{condNodeID}"'][2], self.nextLines[-1]])
